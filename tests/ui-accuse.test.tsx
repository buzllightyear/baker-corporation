...
describe('accuse dialog', () => {
  ...
  it('three selects, coarse miss count reported (never the slot), second miss ends the case', () => {
    ...
    const w = screen.getByRole('status').textContent!; expect(w).toMatch(/1 of the three parts/); expect(w).not.toMatch(/How/); expect(useGame.getState().state!.accusationsLeft).toBe(1);
  });
