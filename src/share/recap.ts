export interface RecapShare { episodeId: string; timeUsed: number; watsonCalls: number; accusations: number; verdict: 'solved' | 'failed' | null; visited: string[]; unvisited: string[]; order: { cardId: string; by: 'holmes' | 'watson'; at: number }[] }
const KEYS: (keyof RecapShare)[] = ['episodeId', 'timeUsed', 'watsonCalls', 'accusations', 'verdict', 'visited', 'unvisited', 'order'];
