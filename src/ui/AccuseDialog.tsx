...
export function AccuseDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const res = r.result as { verdict: string | null; mismatches?: number };
    if (r.state.verdict || !res.mismatches) { onClose(); return; }
    setWrong(T.accuseClose[lang].replace('{n}', String(res.mismatches)));
  };
  return (
    <div className="dialog-bg" onClick={onClose}>
      <form className="dialog frame dialog-accuse" onClick={(e) => e.stopPropagation()} onSubmit={submit}>
        ...
        <div className="dialog-body">
          ...
          {wrong && <div className="wrong" role="status">{wrong}</div>}
