...
const EXIT_POS: Record<'left' | 'right' | 'ahead' | 'back', React.CSSProperties> = {
  ...
  ahead: { left: '50%', top: 14, transform: 'translateX(-50%)' }, back: { left: '50%', bottom: 14, transform: 'translateX(-50%)' },
};
  ...
  return (
    <div className={'stage' + (img === 'ok' ? ' has-art' : '')} onMouseMove={onMove} onMouseLeave={() => setPar({ x: 0, y: 0 })}>
      ...
      {sc.people.map((p, i) => { const h = art?.people?.[p.id] ?? peoplePos(i, sc.people.length); const src = PORTRAIT[p.id]; return (
        <button key={p.id} className={'hot person' + (person === p.id ? ' on' : '')} style={{ left: `${h.x}%`, top: `${h.y}%` }} aria-label={pick(p.name, lang)} onClick={() => { setLook(null); setPerson(person === p.id ? null : p.id); }}>
