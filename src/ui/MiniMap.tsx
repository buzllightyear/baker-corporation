  ...
  return (
    <div className="minimap">
      ...
      <div className="mm-grid">
        {ep.places.map((p) => { const people = whoIsHere(ep, st, p.id); const cls = ['room', p.id === here.id ? 'here' : '', here.adjacent.includes(p.id) ? 'adjacent' : '', visited.has(p.id) || p.id === ep.startPlaceId ? 'visited' : '', goal.room === p.id ? 'goal' : ''].join(' ');
