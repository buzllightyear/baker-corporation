...
export const useGame = create<GameStore>((set, get) => ({
  ...
  startEpisode: (id) => { const ep = registry.get(id); if (!ep) throw new Error(`no episode ${id}`); const state = newRun(ep.id, ep.startPlaceId, ep.watsonStartPlaceId); saveRun(state); set({ episode: ep, state, activity: [], watsonReads: 0, lastHearing: null }); },
